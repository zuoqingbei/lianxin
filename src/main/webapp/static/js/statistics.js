...
function professionalStatis() {
    ...
    $.post(contextPath + '/lab/labStatisByFiledAjax', {
...
function proLineStatis() {
    ...
    $.post(contextPath + '/lab/labStatisByFiledAjax', {
        ...
        /*$.each(data,function(index,item){
            $(".pro_line").append('<li><span class="icon"></span>'+item.name+'<span class="number hlnum" style="color: transparent;">'+linkNum[index]+'</span><span class="number allnum" style="color: transparent;">'+item.count+'</span></li>');
...
function labLifeCycleStatis() {
    $.post(contextPath + '/lab/labCarryNumStatisAjax', {
        "labType": labType,
        "productCode": productCode
    }, function (data) {
...
/*                    data: function () {
                        console.log("```````````````实验室数量",data)
                        return statisticSeriesDataData(data)
                    },*/
                    lineStyle: {
                        normal: {
                            width: 2 * bodyScale
