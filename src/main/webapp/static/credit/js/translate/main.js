...
//        	if(!urlTemp){return}
            ...
            if (titles[index]["get_source"].includes("credit_company_shareholder")) {
                ...
                $table.bootstrapTable({
                    ...
                    locales: 'zh-CN',
                    onLoadSuccess: (data) => {
                        // data数据为空时，data.rows有的是[]，有的是null
                         tableNum++;
                        if (this.idArr.length === tableNum) {
                            //中文表格数据加载完成，可以点翻译按钮啦
                            $("#translateBtn").removeClass("disable")
...
//	    			async:false,
                success: (data) => {
                    if(!data.rows){
                        console.warn('~~表单返回数据为空');
                        return
                    }
                    temp = data
                    console.log("~~",url,data.rows)
...
//    					$("#"+id).attr("en_bak",obj[anotherId])
            ...
            formIndexEn.forEach((item, index) => {
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        if (temp.rows === null || !temp.rows || temp.rows.length === 0) {
                            return
                        }
                        arr.forEach((item, index) => {
                            if ($(item).siblings(".radio-con").length !== 0) {
    ...
    initFloat() {
        ...
        let cw_bottom_html = ''
        let tableCwId = []
        // console.log('~~cw_title',cw_title)
        // console.log('~~cw_contents',cw_contents)
        cw_title.forEach((item, index) => {
            ...
            if (item.sort === 1) {
                ...
                cw_top_html += `<div class="top-html mx-4">
    								...
    								<div class="cw-unit">
    									<div class="form-inline my-3">
    										<label style="font-weight:600;margin-left:60%" class="mr-3">${this_content[4] ? this_content[4].temp_name : ''}</label>
    										<select disabled="disabled" class="form-control mr-3 moneySel" id="${this_content[4] ? this_content[4].temp_name : ''}cw" style="width:10rem" name=${this_content[4] ? this_content[4].temp_name : ''}>${moneyStr}</select>
                ...
                cw_bottom_html += `<div class="bottom-html"><div class="cw-bottom p-4">
    								...
    								<select disabled="disabled" class="form-control my-3 ${this_content[10] ? this_content[10].temp_name : ''}" id="${this_content[10].column_name}cw" name="${this_content[10].column_name}">${options}</select>
...
//            	    $(".triggerModal").trigger("click")
                ...
                modules.forEach((item, index) => {
                    ...
                    switch (smallModileType) {
                        case '0':
                            console.log('~~smallModileType',smallModileType)
                            //表单类型
                    ...
                    if (!(item_en&&item_en.title&&item_en.title.temp_name)) {
                        return
                    }
                    let smallModileTypeEn = item_en.smallModileType;
                    let temp_name_en = item_en.title.temp_name;
                    if(temp_name_en&&(temp_name_en === 'Key Fiancial Items' || temp_name_en === 'industry_analysis' || temp_name_en.includes('质检意见'))){
        				return
            		}
                    if (['Key Fiancial Items', 'industry_analysis', '质检意见'].includes(temp_name_en)) {
                        return
                    }
                    if (temp_name_en && item_en.title.float_parent) {
                        return
                    }
                    if (!(temp_name_en === null || temp_name_en === "") || !item_en.title.float_parent) {
                        if (temp_name_en === null || temp_name_en === "" || item_en.title.float_parent) {
                            contentHtml += `<div class="bg-f pb-4 mb-3"  ><a style="display:none" class="l-title" name="anchor${item_en.title.id}" id="titleEn${index}">${temp_name_en}</a>`
                        } else if (smallModileTypeEn === '10') {
                            //财务模块
                            //                		_this.cwGetSource = item.title.get_source;
                            //                		_this.cwAlterSource = item.title.alter_source;
                            //                		_this.cwDeleteSource = item.title.remove_source;
                            contentHtml += `<div class="bg-f pb-4 mb-3 gjcw"><a class="l-title cwmodal" name="anchor${item.title.id}" id="titleCwEn${index}">${temp_name_en}</a>`
                        } else if (smallModileTypeEn !== '-2' && smallModileTypeEn !== '5' && smallModileTypeEn !== '2') {
                            contentHtml += `<div class="bg-f pb-4 mb-3"><a class="l-title" name="anchor${item.title.id}" id="titleEn${index}">${temp_name_en}</a>`
                        }
                    }
                    ...
                    switch (smallModileTypeEn) {
                        case '0':
                            console.log('~~~smallModileTypeEn',smallModileTypeEn)
                            //表单类型
                        ...
                        case '1':
                            ...
                            if (temp_name_en && (temp_name_en.includes('股东详情') || temp_name_en.includes('管理层'))) {
                                //法人，自然人股东 不换行
                                contentHtml += `<div class="table-content1 nowrap" style="background:#fff">
            					<table id="table${index}"
            					data-toggle="table"
                            ...
                            } else if (!temp_name_en.includes('质检意见')) {
                                contentHtml += `<div class="table-content1" style="background:#fff">
...
//            			contentHtml += Public.fileConfig(item,_this.rows)
                        ...
                        case '5':
                            ...
                            if (item_en.title.column_name === 'save') {
                                bottomBtn += `<button id="translateBtn" class="btn btn-primary ml-4 disable">翻译</button><button id=${item_en.title.column_name} class="${className}">${temp_name_en}</button>`
                            } else {
                                bottomBtn += `<button id=${item_en.title.column_name} class="${className}">${temp_name_en}</button>`
                            }
                            break;
                        case '6':
                            //信用等级
                        ...
                        case '9':
                            ...
                            if (item_en.contents.length === 0) {
                                ...
                                _this.notMoneyFloatHtmlEn[index] = `<div class="form-group form-inline p-4">
				                          <label >${temp_name_en === null ? '' : temp_name_en}</label>
    ...
    bottomBtnEvent() {
        ...
        tableTitlesEn.forEach((item, index) => {
            if(!item.alter_source.split('?')[1]){return}
...
//    		console.log(alterSource)
                ...
                if (alterSource === null || alterSource === '' || alterSource === "alterFinanceOneConfig") {
                    return
                }
                let url = BASE_PATH + 'credit/front/ReportGetData/' + alterSource.split("*")[0];
                let dataJson = []
    		...
    		if(!_this.formDataArr[index]){
                ...
                $(".position-fixed").on("click", "#translateBtn", (e) => {
                    ...
                    console.log('~~要翻译的表单',_this.formDataArr[index])
                    $.ajax({
                        url,
                        type: 'post',
                        data: {
                ...
                if (dataJsonObj["company_id"] && !dataJsonObj["company_id"] || !_this.formDataArr[index]) {
                    return
                }
                //点击保存按钮
...
//    			InitObjTrans.saveCwConfigInfo(_this.cwConfigAlterSource,_this.rows);
    ...
    showTranslateMadal() {
        ...
        $("#submit_revise").click(() => {
            ...
            //验证三个输入框有内容后才提交
            /*let verification = $('#modal_revise .modal-body input').every(function (index,item) {
				return $(item).val()
			});
            ...
            if (error_phrase_en && correct_phrase_en && correct_phrase_ch) {
