...
public class KpiService {
	...
	public double doFor1(String companyId,  String tableName, String column1, double price) {
		try {
			...
			String flagStr = Db.queryColumn(
					"select " + column1 + " from " + tableName + " where del_flag=0 and companyId=?  ",
	...
	public double doFor2(String companyId, double price, int sourceYeal) {
		try {
			...
			List<String> flagStr = Db.query(
					"select date1,date2 from credit_company_financial_statements_conf where del_flag=0 and companyd_id=?  ",
					Arrays.asList(new String[] { companyId   }));
	 ...
	 * @return
	 */
	public double getKpi(String roleId,String orderId) {
		if(StrUtils.isEmpty(orderId,roleId)) {
			 return 0;
		}
		...
		// 若是国外
		if(!isInland) {
 			CreditKpiPrice kpiDict = CreditKpiPrice.dao.findFirst("select normal_price,urgent_price,extra_price from credit_kpi_krice where del_flag=0 and formula_mode=4 and role_id="+roleId);
 			if(speed==149) {
 				return kpiDict.getDouble("normal_price");
 			}else if(speed==150) {
 				return kpiDict.getDouble("urgent_price");
 			}else if(speed==151) {
 				return kpiDict.getDouble("extra_price");
 			}
 			return 0;
		}else {
