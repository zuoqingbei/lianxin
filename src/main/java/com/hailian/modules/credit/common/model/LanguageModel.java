...
public class LanguageModel extends BaseProjectModel<LanguageModel>{
	...
	public Page<LanguageModel> pagerOrder(int pageNumber, int pagerSize, List<Object> keywords, String orderBy,String searchType, BaseProjectController c) {
		StringBuffer selectSql = new StringBuffer(" SELETE c.* ");
		selectSql.append(" s1.detail_name AS report, ");
		...
		fromSql.append(" LEFT JOIN sys_dict_detail s1 ON c.report_id = s1.detail_id ");
		...
		if(LanguageController.searchById.equals(searchType)){
			...
			params.add(keywords);//传入的参数
			return LanguageModel.dao.paginate(new Paginator(pageNumber, pagerSize), selectSql.toString(),fromSql.toString(), params.toArray());
		}
		//若为其他搜索类型
