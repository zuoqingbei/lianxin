		...
		Page<CreditOrderInfo> page = CreditOrderInfo.dao
				.paginate(
						...
						"select t.*,u.name as customName,c.name as country,c.id as countryId,s.realname as createName,s8.realname as reportName,s9.realname as translateName,s0.realname as analyzeName"
								+ ",s2.detail_name as continentName,s3.name as reportType,s4.detail_name as reportLanguage,"
								+ "s5.detail_name as speed,s5.detail_id as speedId,s6.detail_name as orderType,s7.detail_name as statusName,c1.price as price,c2.name as companyName,c2.name_en as englishName,s10.use_time as useTime  ",
						sql.toString(), params.toArray());
...
//	}
	...
	* @TODO
	...
	public CreditOrderInfo isTheSameOrder(String company_id,String report_type,String report_language, BaseProjectController c) {
		String sql="select t.* from credit_order_info t where t.right_company_name_en=? and t.report_type=? and t.report_language=? and (t. is_fastsubmmit='-1' or t. is_fastsubmmit is null or t. is_fastsubmmit='') and t.del_flag=0 and t.status='311' order by t.create_date desc";
