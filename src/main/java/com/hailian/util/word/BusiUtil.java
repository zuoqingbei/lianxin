...
public class BusiUtil extends BaseWord{
	 public static Logger log = Logger.getLogger(BaseBusiCrdt.class);
	     ...
	     * @throws Exception
	     */
	    public static void reportTableCh(CreditOrderInfo order,Integer userid) throws Exception {
	    	HashMap<String, Object> m2=reportTableChMethod(order, ReportTypeCons.BUSI_ZH, "612", userid, null,true,true);
	    	HashMap<String, Object> m1=reportTableChMethod2(order, ReportTypeCons.BUSI_EN, "613", userid, null,true,true);
	    	if("true".equals(m1.get("status")+"")&&"true".equals(m2.get("status")+"")){
	    		...
	    		String msg=m1.get("msg")+";"+m2.get("msg");
	    		sendErrorEmail(order,new RuntimeException(msg));
	    	}
	    }
	    public static List<Map<String, String>> sortFiles(Map<String, String> fileList){
	    	List<Map<String, String>> f = new ArrayList<>();
	    	Map<String, String> files=new HashMap<String, String>();
	    	for(String key:fileList.keySet()){//keySet获取map集合key的集合  然后在遍历key即可
	    		if(StringUtils.isNotBlank(key)&&(key.endsWith(".doc")||key.endsWith(".docx"))){
	    			files.put(key,fileList.get(key));
	    		}
	    	}
	    	for(String key:fileList.keySet()){//keySet获取map集合key的集合  然后在遍历key即可
	    		if(StringUtils.isNotBlank(key)&&(key.endsWith(".xls")||key.endsWith(".xlsx"))){
	    			files.put(key,fileList.get(key));
	    		}
	    	}
	    	f.add(files);
	    ...
	    public static void reportTableEn(CreditOrderInfo order,Integer userid) throws Exception {
	    	HashMap<String, Object> m2=reportTableChMethod(order, ReportTypeCons.BUSI_ZH, "613", userid, null,true,false);
	    	HashMap<String, Object> m1=reportTableChMethod(order, ReportTypeCons.BUSI_EN, "613", userid, null,true,false);
	    	if("true".equals(m1.get("status")+"")&&"true".equals(m2.get("status")+"")){
	    		Map<String, String> fileList = new HashMap<String, String>();
	    		Map<String, String> file1=(Map<String, String>) m1.get("files");
	    		Map<String, String> file2=(Map<String, String>) m2.get("files");
	    ...
	    public static HashMap<String, Object> reportTableChMethod2(CreditOrderInfo order,String reportType,String sysLanguage,
	    		Integer userid,Map<String,Object> extend,boolean needExc,boolean isCh) throws Exception {
	        ...
	        if(ReportTypeCons.BUSI_EN.equals(reportType)){
	        	String sql = "select * from credit_company_info t where t.order_id = ? and t.sys_language=612";
	        	CreditCompanyInfo companyInfo = CreditCompanyInfo.dao.findFirst(sql,orderId);
	        ...
	        //订单公司名称
        	map.put("company", companyInfo.getStr("name"));
	        //报告速度
	        ...
	        synchronized (o){
	            {
	                ...
	                if(!isUploadSuccess){
	                	...
	                	result.put("msg", " 服务器文件生成路径:"+_prePath+".docx; file.exists="+file.exists()+"; file.length="+file.length()+"; wordPath="+_pre + wordPath);
	                }{
	                    if(excelPath!=null&&excelPath.size()!=0&&needExc) {
	                        int count = 0;
	    ...
	    public static HashMap<String, Object> reportTableChMethod(CreditOrderInfo order,String reportType,String sysLanguage,
	    		Integer userid,Map<String,Object> extend,boolean needExc,boolean isCh) throws Exception {
	        HashMap<String, Object> map = new HashMap<String, Object>();
	        //报告名称
	        String reportNameCh = Db.query("select name from credit_report_type where del_flag=0 and id=?",reportType)+"";
	        ...
	        if(confs!=null){
	            for (CreditReportModuleConf conf : confs) {
	              if(isNotNull(sourceField)&&sourceField.equals(conf.get("column_name"))){
	                  if(isSelect){
	                      targetName = dictIdToString(targetName, reportType, sysLanguage);
	                  }
	    ...
	    private static String getValue(String reportType, String sysLanguage,
			...
			if ("select".equals(fieldType)) {
			    value = !"".equals(value) ? dictIdToString(value, reportType, sysLanguage) : "";
			}
			//下拉多选
			    ...
			    String selName = "";
			    for(String val : vals){
			        selName += dictIdToString(val, reportType, sysLanguage) + ",";
			    }
			    if(selName.contains(",")) {
	    ...
	    public static List<RowRenderData> financial(CreditCompanyFinancialStatementsConf financialConf,HashMap<String, Object> map,String reportType) {
	        ...
	        if("21".equals(reportType)){
	            ...
	            String currencyUnit396 = dictIdToStringSpecified(currencyUnitId,"detail_content");
	    ...
	    public static String financialEval(CreditCompanyFinancialStatementsConf statementsConf,String reportType,String sysLanguage) {
	        ...
	        str.append("盈利能力：" + (!"".equals(profSumup) ? dictIdToString(profSumup,reportType,sysLanguage) : ""));
	        str.append("\n");
	        str.append(profDetail);
	        str.append("\n");
	        str.append("周转能力：" + (!"".equals(liquSumup) ? dictIdToString(liquSumup,reportType,sysLanguage) : ""));
	        str.append("\n");
	        str.append(liquDetail);
	        str.append("\n");
	        str.append("融资能力：" + (!"".equals(leverSumup) ? dictIdToString(leverSumup,reportType,sysLanguage) : ""));
	        str.append("\n");
	        str.append(leverDetail);
	        str.append("\n");
	        str.append("目标公司的总体财务状况：" + (!"".equals(overSumup) ? dictIdToString(overSumup,reportType,sysLanguage) : ""));
	      /*  str.append("\n");
